...
class PresenterRegistrationTwo {
    ...
    @SuppressLint("CheckResult")
    fun doNameVerify(
            contextOBJ: Context, token: String, reqNameVerify: ReqNameVerify, callBackOBJ: ICallBackRegisterTwo
    ){
        //val sharedPref = SharedPref(contextOBJ)
        RetrofitHelper.getRetrofitToken(BaseService::class.java,token).doNameVerify(reqNameVerify)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .map { responseBody ->
                    responseBody
                ...
                .subscribe({ response ->
                    ...
                    try
                    {
                        ...
                        {
                            callBackOBJ.onErrorRegistrationTwo(response.commonStatusModel.message)
                        }
